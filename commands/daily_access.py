...
async def setup(bot):
    ...
    async def daily_access_channel(
        ...
        if start_hour >= end_hour:
            await interaction.followup.send("❌ Start hour must be before end hour!", ephemeral=True)
            return
    ...
    async def remove_daily_channel(
        ...
        if not cog or channel.id not in cog.channel_schedules:
            await interaction.followup.send(f"❌ No daily schedule found for {channel.mention}!", ephemeral=True)
            return
    ...
    async def list_daily_channels(interaction: discord.Interaction):
        ...
        if not cog or not cog.channel_schedules:
            await interaction.followup.send("📋 No daily channel schedules configured.", ephemeral=True)
            return
